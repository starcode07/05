...
public class ListaDoble {
    ...
    public void insertarUlt(int x) {
        if (estaVacia()) {
            primero = new NodoDoble(null, x, null);
            ultimo = primero;
        } else {
            ultimo.proximo = new NodoDoble(ultimo, x, null);
    ...
    public void insertarNodo(int x, NodoDoble anteriorProx, NodoDoble prox) {
        if (anteriorProx == null) {
            primero = new NodoDoble(anteriorProx, x, prox);
        } else {
            anteriorProx.proximo = new NodoDoble(anteriorProx, x, prox);
        }
        cantElem++;
    }
    ...
    public void eliminarIesimo(int i) {
        if (i == 0) { // Eliminando el primer elemento
            primero = primero.proximo;
        } else {
            NodoDoble anterior = primero;
            for (int j = 1; j < i; j++) { // Avanzar hasta el nodo i
                anterior = anterior.proximo;
            }
            anterior.proximo = anterior.proximo.proximo; // Saltamos el nodo que queremos eliminar
        }
        cantElem--;
    }
